...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("classifications_.txt", np.float32)                  # read in training classifications
    ...
    try:
        npaFlattenedImages = np.loadtxt("flattened_images_.txt", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        # possiblePlate.imgThresh = cv2.resize(possiblePlate.imgThresh, (0, 0), fx = 1.7, fy = 1.7)
        # Resize the image - change width to 500
        possiblePlate.imgThresh == imutils.resize(possiblePlate.imgThresh, width=500)
        ...
        if (len(listOfListsOfMatchingCharsInPlate) == 0):			# if no groups of matching chars were found in the plate
            ...
            if Main.showSteps == True: # show steps ###############################################
                print("No chars found in plate number " + str(intPlateCounter) + " = (none), click on any image and press a key to continue . . .")
        ...
        possiblePlate.strChars.append(recognizeCharsInPlate(possiblePlate.imgThresh, longestListOfMatchingCharsInPlate))
        result.append(possiblePlate.strChars)
        #print('result: ' , result)

        if Main.showSteps == True: # show steps ###################################################
            print("chars found in plate number " + str(intPlateCounter) + " = " + str(possiblePlate.strChars) + ", click on any image and press a key to continue . . .")
            intPlateCounter = intPlateCounter + 1
            cv2.waitKey(0)
        # end if # show steps #####################################################################
    ...
    return listOfPossiblePlates, result
# end function
